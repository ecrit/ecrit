package html.to.latex.converter;

import html.to.latex.config.Configuration;
...
import html.to.latex.io.IWriter;
import html.to.latex.io.OutputFileWriter;
import html.to.latex.io.OutputStringWriter;
import html.to.latex.main.ProgramInput;
import html.to.latex.parser.ElementEnd;
import html.to.latex.parser.ElementStart;
 ...
 * {@link Converter#commonElementStart(ElementStart) commonElementStart()} and
 * {@link Converter#commonElementEnd(ElementEnd, ElementStart) commonElementEnd()} methods. Elements
 * requiring special care during the conversion are converted by calling special methods like
 * {@link Converter#tableRowStart(ElementStart) tableRowStart() }.
 */
public class Converter {
	...
	public Converter() throws FatalErrorException{
		_config = new Configuration(new ProgramInput());
	 ...
	 *             tag not found in the configuration
	 */
	public void commonElementEnd(ElementEnd element, ElementStart es) throws IOException,
		NoItemException{
		ElementConfigItem item = _config.getElement(element.getElementName());
	...
	private String convertLaTeXSpecialChars(String str){
		str =
			str.replace("\\", "@-DOLLAR-\\backslash@-DOLLAR-").replace("&#", "&@-HASH-")
				.replace("$", "\\$").replace("#", "\\#").replace("%", "\\%")
				.replace("~", "\\textasciitilde").replace("_", "\\_")
				.replace("^", "\\textasciicircum").replace("{", "\\{").replace("}", "\\}")
					...
					if (entity.charAt(0) == '#') {
						try {
							...
							if ((entity.charAt(1) == 'x') || entity.charAt(1) == 'X') {
								entityNum =
									Integer.valueOf(entity.substring(2, entity.length()), 16);
							} else {
	 ...
	 *             tag not found in the configuration
	 */
	public void tableCellEnd(ElementEnd element, ElementStart e) throws IOException,
		NoItemException{
	...
	public void bodyEnd(ElementEnd element, ElementStart es) throws IOException, NoItemException{
		if (!_bibliography.isEmpty()) {
			...
			for (Iterator<Entry<String, String>> iterator = _bibliography.entrySet().iterator(); iterator
				.hasNext();) {
