...
public class CSSStyle {
	...
	/**
	 * Sets {@link CSSStyle#_start start} and {@link CSSStyle#_end end} commands for the style on
	 * the basis of style properties and program configuration.
	 * 
	 * @param conf
	...
	public void makeLaTeXCommands(Configuration conf){
		...
		for (Iterator<Entry<String, String>> iterator = _properties.entrySet().iterator(); iterator
			.hasNext();) {
	...
	/*
	 * public void inheritProperties(Configuration conf) { String elementName; if
	 * (_name.contains("#")) elementName = _name.substring(0, _name.indexOf('#')); else if
	 * (_name.contains(".")) elementName = _name.substring(0, _name.indexOf('.')); else return;
	 * 
	...
	private void colorProperty(Configuration conf){
		...
		if (color.startsWith("#")) {
			...
			_start +=
				"{\\color[rgb]{" + format.format(nums[0]) + "," + format.format(nums[1]) + ","
	...
	public void fontFamilyProperty(Configuration conf){
		...
		for (int i = 0; i < fonts.length; ++i) {
			try {
				CSSPropertyConfigItem item =
					conf.getPropertyConf("font-family" + "-" + fonts[i].trim());
				_start += item.getStart();
				_end = item.getEnd() + _end;
				break;
			} catch (NoItemException e) {
