...
public class AppModelHelper {
	...
	public static void initAppModel(EModelService modelService,
			Resource appModelResource) {
		AppModelHelper.application = (MApplication) appModelResource
				.getContents().get(0);
	...
	public static ApplicationDocumentation getApplicationDocumentation() {
		...
		return new ApplicationDocumentation(
				persistedState.get(Constants.PERSISTENT_STATE_ABOUT),
				...
				persistedState
						.get(Constants.PERSISTENT_STATE_PERSPECTIVE_SWITCH));
	}

	public static PerspectiveStackDocumentation getPerspectiveStackDocumentation(
			String elementId) {
		if (elementId == null || elementId.length() == 0) {
			throw new IllegalArgumentException();
		}
		...
		if (mae == null) {
			...
			for (TreeIterator<EObject> i = appModelResource.getAllContents(); i
				...
				if (object instanceof MApplicationElement) {
					...
					if ((maeTemp != null)
							&& (maeTemp.getElementId() != null)
							&& maeTemp.getElementId().equalsIgnoreCase(
									elementId)) {
						mae = maeTemp;
					}
				}
